...
import { bridgeConnect, isValidPurpose, managePermissionFor, resolveBridgeConfigByPurpose } from "@/lib/whatsapp/bridge";
...
export async function POST(request: NextRequest) {
  ...
  try {
    await bridgeConnect(config);
    await logAudit({ actorUserId: profile.id, action: "whatsapp.connect", entityType: "whatsapp_bridge" });
