...
import { bridgeDeleteSession, isValidPurpose, managePermissionFor, resolveBridgeConfigByPurpose } from "@/lib/whatsapp/bridge";
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    await bridgeDeleteSession(config);
    await logAudit({ actorUserId: profile.id, action: "whatsapp.session_delete", entityType: "whatsapp_bridge" });
